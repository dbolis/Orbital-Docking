...
w_t=transpose(x(1:timeAll(1),2:4));
rho=transpose(x(1:timeAll(1),16:18));
drho=transpose(x(1:timeAll(1),19:21));
% q_r=error(4:7,:)+q_d.*ones(4,timeAll(1));
% dq_r=derror(4:7,:);
% rho_out=error(1:3,:)+rho_d.*ones(3,timeAll(1));
...
dT=[0, -dq_r(4,i), dq_r(3,i); dq_r(4,i), 0, -dq_r(2,i);-dq_r(3,i), dq_r(2,i), 0] + dq_r(1,i)*eye(3);
P=inv(T);
dP=inv(dT);
Istar=transpose(P)*Ic*P;

IcPdq_r=Ic*P*dq_r(2:4,i);
Pdq_r=P*dq_r(2:4,i);
Arw_t=Ar*w_t(1:3,i);
twoPdq_r=2*P*dq_r(2:4,i);
...
Nstar=transpose(P)*([0, -Pdq_r(3), Pdq_r(2); Pdq_r(3), 0, -Pdq_r(1); -Pdq_r(2), Pdq_r(1), 0]*...
    Ic*Ar*w_t(1:3,i))+transpose(P)*([0, -Arw_t(3), Arw_t(2); Arw_t(3), 0, -Arw_t(1);-Arw_t(2), Arw_t(1), 0]*Ic*P*dq_r(2:4,i))+...
